...
$descripcionCategoria=$categoria->getDescripcionCategoria();
      ...
      try {
          $sql= "INSERT INTO `categoria`( `idcategoria`, `descripcionCategoria`)"
          ."VALUES ('$idcategoria','$descripcionCategoria')";
  ...
  public function select($categoria){
      ...
      try {
          $sql= "SELECT `idcategoria`, `descripcionCategoria`"
          ."FROM `categoria`"
          ."WHERE `idcategoria`='$idcategoria'";
          $data = $this->ejecutarConsulta($sql);
          for ($i=0; $i < count($data) ; $i++) {
          $categoria->setIdcategoria($data[$i]['idcategoria']);
          $categoria->setDescripcionCategoria($data[$i]['descripcionCategoria']);
...
$descripcionCategoria=$categoria->getDescripcionCategoria();
      ...
      try {
          $sql= "UPDATE `categoria` SET`idcategoria`='$idcategoria' ,`descripcionCategoria`='$descripcionCategoria' WHERE `idcategoria`='$idcategoria' ";
         return $this->insertarConsulta($sql);
      } catch (SQLException $e) {
          throw new Exception('Primary key is null');
      }
